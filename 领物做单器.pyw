...
class LandwuGuiApp:
    ...
    def open_settings_window(self) -> None:
        ...
        ttk.Label(form, text="软件更新").grid(row=6, column=0, sticky="w", pady=4)
        self._add_button(form, 6, 1, "检查更新", self.check_for_update, style="Ghost.TButton")
        ttk.Label(form, text=f"当前版本：{APP_VERSION}", background="#FFFFFF", foreground="#6C757D").grid(
            row=6, column=2, columnspan=4, sticky="w", padx=(8, 0), pady=4
        )

        ttk.Separator(form).grid(row=7, column=0, columnspan=6, sticky="ew", pady=12)
        ttk.Checkbutton(
            form,
            text="开启自动刷新",
        ...
        ttk.Label(form, text="模式").grid(row=8, column=1, sticky="e", pady=4)
        ttk.Combobox(
            form,
            ...
            width=10,
            state="readonly",
        ).grid(row=8, column=2, sticky="w", padx=(8, 0), pady=4)
        ttk.Label(form, text="间隔分钟").grid(row=8, column=3, sticky="e", padx=(10, 0), pady=4)
        ttk.Entry(form, textvariable=self.auto_refresh_interval_var, width=8).grid(row=8, column=4, sticky="w", padx=(8, 0), pady=4)
        ttk.Label(form, text="每天 HH:MM").grid(row=9, column=3, sticky="e", padx=(10, 0), pady=4)
        ttk.Entry(form, textvariable=self.auto_refresh_time_var, width=8).grid(row=9, column=4, sticky="w", padx=(8, 0), pady=4)
        ttk.Button(form, text="应用定时", command=self.apply_auto_refresh_settings).grid(row=9, column=5, sticky="e", pady=4)
        ttk.Label(form, textvariable=self.auto_refresh_status_var, background="#FFFFFF", foreground="#6C757D").grid(
            row=10, column=0, columnspan=6, sticky="w", pady=(10, 0)
        )

        footer = ttk.Frame(form, style="Surface.TFrame")
        footer.grid(row=11, column=0, columnspan=6, sticky="e", pady=(14, 0))
