...
def get_default_auth_state_file() -> Path:
    return Path(r"D:\auth-state-v1.json")


def get_app_settings_file() -> Path:
    local_app_data = os.environ.get("LOCALAPPDATA")
    base_dir = Path(local_app_data) if local_app_data else Path.home() / "AppData" / "Local"
    return base_dir / "领物做单器" / "settings.json"
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--auth-source", choices=[AUTH_SOURCE_AUTO, AUTH_SOURCE_BROWSER, AUTH_SOURCE_FILE], default=AUTH_SOURCE_AUTO)
...
def build_runtime_args(
    *,
    ...
    auth_source: str = AUTH_SOURCE_AUTO,
...
class LandwuGuiApp:
    def __init__(self, root: tk.Tk):
        ...
        self.auth_source_var = tk.StringVar(value="自动（文件优先）")
    ...
    def _scroll_canvas(self, canvas: tk.Canvas, event) -> str | None:
    ...
    def _show_image_preview_window(self, image_items: list[dict[str, Any]], missing_order_nos: list[str]) -> None:
        ...
        ttk.Button(toolbar, text="打开目录", command=lambda: os.startfile(self._current_output_dir())).pack(side="right")
        ...
        for item in image_items:
            ...
            if high_url:
                high_btn = ttk.Button(row_header, text="查看高清", command=lambda url=high_url: os.startfile(url))
    ...
    def copy_auth_sync_userscript(self) -> None:
        ...
        messagebox.showinfo(
            ...
            "随后回到这里点击“接收登录态3分钟”，再打开或刷新 Landwu 页面即可同步。",
        )
    ...
    def open_settings_window(self) -> None:
        ...
        ttk.Label(form, text="成分数据库").grid(row=2, column=0, sticky="w", pady=4)
        ttk.Entry(form, textvariable=self.composition_db_folder_var).grid(row=2, column=1, columnspan=3, sticky="ew", padx=(8, 0), pady=4)
        ttk.Button(form, text="选择目录", command=self._choose_composition_db_folder).grid(
            row=2, column=4, columnspan=2, sticky="w", padx=(12, 0), pady=4
        )

        ttk.Label(form, text="登录态来源").grid(row=3, column=0, sticky="w", pady=4)
        ...
        auth_source.grid(row=3, column=1, sticky="w", padx=(8, 0), pady=4)
        ttk.Button(form, text="登录态说明", command=self.show_auth_help).grid(row=3, column=2, sticky="w", padx=(8, 0), pady=4)
        ttk.Button(form, text="查看登录态", command=self.fetch_auth).grid(row=3, column=3, sticky="w", padx=(8, 0), pady=4)
        ttk.Label(form, text="下单余额").grid(row=3, column=4, sticky="e", padx=(12, 4), pady=4)
        ttk.Label(form, textvariable=self.balance_var, style="SettingsBalance.TLabel").grid(row=3, column=5, sticky="w", pady=4)

        ttk.Label(form, text="同步文件").grid(row=4, column=0, sticky="w", pady=4)
        ttk.Entry(form, textvariable=self.auth_file_var).grid(row=4, column=1, columnspan=2, sticky="ew", padx=(8, 0), pady=4)
        ttk.Button(form, text="选择文件", command=self._choose_auth_file).grid(row=4, column=3, sticky="w", padx=(8, 0), pady=4)
        ...
        ttk.Label(form, text="同步脚本").grid(row=5, column=0, sticky="w", pady=4)
        ...
        ttk.Label(form, text="首次使用：复制后粘贴到 Tampermonkey 并启用。", background="#FFFFFF", foreground="#6C757D").grid(
            row=5, column=3, columnspan=3, sticky="w", padx=(8, 0), pady=4
        )
        ttk.Label(form, textvariable=self.auth_sync_status_var, background="#FFFFFF", foreground="#6C757D").grid(
            row=6, column=1, columnspan=5, sticky="w", padx=(8, 0), pady=(0, 4)
        )
        ttk.Label(form, text="软件更新").grid(row=7, column=0, sticky="w", pady=4)
        self._add_button(form, 7, 1, "检查更新", self.check_for_update, style="Ghost.TButton")
        ttk.Label(form, text=f"当前版本：{APP_VERSION}", background="#FFFFFF", foreground="#6C757D").grid(
            row=7, column=2, columnspan=4, sticky="w", padx=(8, 0), pady=4
        )

        ttk.Separator(form).grid(row=8, column=0, columnspan=6, sticky="ew", pady=12)
        ttk.Checkbutton(
            form,
            text="开启自动刷新",
        ...
        ttk.Label(form, text="模式").grid(row=9, column=1, sticky="e", pady=4)
        ttk.Combobox(
            form,
            ...
            width=10,
            state="readonly",
        ).grid(row=9, column=2, sticky="w", padx=(8, 0), pady=4)
        ttk.Label(form, text="间隔分钟").grid(row=9, column=3, sticky="e", padx=(10, 0), pady=4)
        ttk.Entry(form, textvariable=self.auto_refresh_interval_var, width=8).grid(row=9, column=4, sticky="w", padx=(8, 0), pady=4)
        ttk.Label(form, text="每天 HH:MM").grid(row=10, column=3, sticky="e", padx=(10, 0), pady=4)
        ttk.Entry(form, textvariable=self.auto_refresh_time_var, width=8).grid(row=10, column=4, sticky="w", padx=(8, 0), pady=4)
        ttk.Button(form, text="应用定时", command=self.apply_auto_refresh_settings).grid(row=10, column=5, sticky="e", pady=4)
        ttk.Label(form, textvariable=self.auto_refresh_status_var, background="#FFFFFF", foreground="#6C757D").grid(
            row=11, column=0, columnspan=6, sticky="w", pady=(10, 0)
        )

        footer = ttk.Frame(form, style="Surface.TFrame")
        footer.grid(row=12, column=0, columnspan=6, sticky="e", pady=(14, 0))
    ...
    def _format_payment_card_size_summary(row: dict[str, Any]) -> str:
        ...
        for detail in row.get("detail") or []:
            if not isinstance(detail, dict):
                continue
            size = display_size_name(detail.get("size") or detail.get("spec_size") or detail.get("goods_size") or "").strip()
            if not size:
                continue
