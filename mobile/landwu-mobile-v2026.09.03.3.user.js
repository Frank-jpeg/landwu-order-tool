...
(function () {
  ...
  function optionId(option, fallback) {
    ...
    return normalizeDbKey(value || fallback || '');
  }
  ...
  function eachNamedOption(options, callback) {
    if (Array.isArray(options)) {
      options.forEach(function (option, index) {
        callback(optionId(option, index), option);
      });
      return;
    }
  ...
  async function getOrderSizeState(item) {
    ...
    var currentSizeId = normalizeDbKey(
      current.size_id || current.sizeId || item.currentSizeId ||
      (item.listDetail && (item.listDetail.size_id || item.listDetail.sizeId))
    );
