...
```text
https://raw.githubusercontent.com/Frank-jpeg/landwu-order-tool/codex/landwu-mobile/mobile/landwu-mobile-v2026.09.04.2.user.js
```
...
匹配优先级为 `SKU_ID -> SKU -> SKC_ID -> SPU_ID`。如果 `target_size` 为空，脚本会根据 `composition` 推断：优先 `人棉`，再 `涤纶/聚酯/polyester`，再 `棉`。手机版 APK 使用全屏工作台脚本 `landwu-mobile-v2026.09.04.2.user.js`。
...
导出脚本只写入 `SKU_ID / SKU / SKC_ID / SPU_ID / composition / target_size`，不会提交原始 Excel/CSV 文件。
