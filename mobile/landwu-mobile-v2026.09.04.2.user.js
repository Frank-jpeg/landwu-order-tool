...
(function () {
  ...
  var SCRIPT_VERSION = '2026.09.04.2';
  ...
  function sizeNameMatches(value, target) {
    var valueText = normalizeText(value);
    var targetText = normalizeText(target);
    if (!valueText || !targetText) return false;
    if (valueText === targetText || normalizedOptionName(valueText) === normalizedOptionName(targetText)) return true;
    return (SIZE_NAME_ALIASES[targetText] || []).some(function (alias) {
      return normalizedOptionName(alias) === normalizedOptionName(valueText);
    });
  }
  ...
  function findNamedOption(options, targetSize) {
    ...
    eachNamedOption(options, function (id, option) {
      if (!found && optionSearchValues(option).some(function (value) { return sizeNameMatches(value, target); })) {
  ...
  async function applyAllSuggestions() {
    ...
    for (var candidateIndex = 0; candidateIndex < candidates.length; candidateIndex += 1) {
      ...
      try {
        ...
        if (!sameById && !sizeNameMatches(candidate.item.currentSize, targetSize)) {
          targets.push(candidate);
        }
      } catch (error) {
        // 详情读取失败时保留手动/提交路径，但不猜测外语值的含义。
        if (!sizeNameMatches(candidate.item.currentSize, targetSize)) targets.push(candidate);
      }
    }
    if (!targets.length) {
  ...
  async function inspectPaymentSizeRisk(rows) {
    ...
    for (var index = 0; index < state.items.length; index += 1) {
      ...
      try {
        ...
        ) || sizeNameMatches(sizeState.currentSizeName, '通用尺码');
  ...
  async function payAllJitOrders() {
    ...
    try {
      ...
      setOperation('正在逐单支付预检', '共 ' + liveIds.length + ' 个 JIT', 1, 3, 'working');
      var successIds = [];
      var failedItems = [];
      for (var payIndex = 0; payIndex < liveIds.length; payIndex += 1) {
        var payId = liveIds[payIndex];
        var payRow = liveRows.find(function (row) { return String(orderIdFor(row)) === String(payId); }) || {};
        var payNo = normalizeText(payRow.order_no) || String(payId);
        try {
          var previewResult = await apiRequest('POST', '/order/getCheckOrder', { ids: String(payId), lange: 'zh' });
          validatePaymentPreview(previewResult.data || {}, [payId]);
          await apiRequest('POST', '/order/orderPay', { ids: String(payId) });
          successIds.push(String(payId));
        } catch (payError) {
          failedItems.push({ orderId: String(payId), orderNo: payNo, error: payError.message || String(payError) });
        }
        setOperation('正在逐单支付预检', '已处理 ' + (payIndex + 1) + '/' + liveIds.length, payIndex + 1, liveIds.length, 'working');
      }

      setOperation('支付已提交', '成功 ' + successIds.length + ' 单，跳过 ' + failedItems.length + ' 单', 3, 3, failedItems.length ? 'warning' : 'success');
      if (failedItems.length) {
        showToast('成功 ' + successIds.length + ' 单，跳过 ' + failedItems.length + ' 单：' + failedItems.slice(0, 2).map(function (item) { return item.orderNo; }).join('、'), 'warning');
      } else {
        showToast('已提交支付 ' + successIds.length + ' 单', 'success');
      }
      await refreshOrders(true);
    } catch (error) {
