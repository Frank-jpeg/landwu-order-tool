...
// @updateURL    https://raw.githubusercontent.com/Frank-jpeg/landwu-order-tool/codex/landwu-mobile/mobile/landwu-mobile-v2026.08.09.2.user.js
// @downloadURL  https://raw.githubusercontent.com/Frank-jpeg/landwu-order-tool/codex/landwu-mobile/mobile/landwu-mobile-v2026.08.09.2.user.js
// ==/UserScript==

(function () {
  'use strict';

  var SCRIPT_VERSION = '2026.08.09.2';
  ...
  var DB_FIELDS = ['SKC_ID', 'SPU_ID', 'SKU'];
  var IMAGE_FIELDS = [
  ...
  function buildSizeItems(rows) {
    ...
    rows.forEach(function (row) {
      ...
      details.forEach(function (detail) {
        ...
        var sku = firstNonEmpty(detail, ['sku', 'productSku', 'product_sku', 'product_sku_id', 'goods_sku']);
  ...
  function findNamedOptionId(options, targetSize) {
    ...
    if (Array.isArray(options)) {
      for (var index = 0; index < options.length; index += 1) {
        if (optionName(options[index]) === target) return normalizeText(options[index].id);
      }
      return '';
    }
    var entries = Object.entries(options || {});
    for (var entryIndex = 0; entryIndex < entries.length; entryIndex += 1) {
      if (optionName(entries[entryIndex][1]) === target) return normalizeText(entries[entryIndex][0]);
    }
    return '';
  }

  function formatOptionNames(options) {
    var values = Array.isArray(options) ? options : Object.values(options || {});
  ...
  async function changeSize(item, targetSize) {
    ...
    if (target === normalizeText(item.currentSize)) return;

    var editDetail = await getEditDetail(item.orderDetailId);
    var sizeMap = editDetail.size || {};
    ...
    var targetSizeId = findNamedOptionId(sizeMap, target);
  ...
  async function handleChangeSize(itemKey, targetSize) {
    ...
    try {
      ...
      state.itemStatus.set(itemKey, { text: '已改为 ' + targetSize, tone: 'success' });
      showToast((item.orderNo || item.sku) + ' 已改为 ' + targetSize, 'success');
    } catch (error) {
  ...
  async function applyAllSuggestions() {
    if (state.busyAction) return;
    var targets = state.items.map(function (item) {
      return { item: item, suggestion: findSuggestion(item) };
    }).filter(function (entry) {
      return entry.suggestion &&
        entry.suggestion.target_size &&
        entry.suggestion.target_size !== normalizeText(entry.item.currentSize);
    });
    if (!targets.length) {
    ...
    for (var index = 0; index < targets.length; index += 1) {
      ...
      try {
        await changeSize(target.item, targetSize);
        successCount += 1;
        state.itemStatus.set(target.item.key, { text: '已改为 ' + targetSize, tone: 'success' });
      } catch (error) {
        failedCount += 1;
    ...
    return [
      ...
      images.map(function (image, index) {
        return [
          ...
          ' data-id="', escapeHtml(item.key), '" data-index="', String(index), '" title="看图">',
  ...
  function injectStyle() {
    ...
    style.textContent = [
      ...
      '#' + ROOT_ID + ' .lw-image-strip { display: flex; gap: 8px; margin-top: 10px; overflow-x: auto; padding-bottom: 2px; -webkit-overflow-scrolling: touch; }',
      '#' + ROOT_ID + ' .lw-thumb { position: relative; flex: 0 0 92px; height: 92px; padding: 0; overflow: hidden; border: 1px solid #d8dde3; border-radius: 6px; background: #f6f8fa; }',
      '#' + ROOT_ID + ' .lw-thumb img { width: 100%; height: 100%; display: block; object-fit: cover; }',
