...
待付款页新增的“一键匹配并提交成分”会直接读取已保存的数据库目录，匹配当前待付款 SKU，二次确认后按“全部关联”批量提交尺码修改；没有可提交目标时只提示结果，不会提交。批量提交前建议先扫一眼“匹配状态”列：尺码与目标一致的行会显示“相同跳过”，不会被提交。
...
同一单元格里并排写了多个 ID 时会自动拆开逐项匹配，分隔符支持 `；`、`;`、`，`、`,` 和空格（例如 6 个颜色的 SKU 挤在一格的导出表）。若“SKU规格”列按同样顺序列出颜色，匹配状态会带上颜色（如 `SKU_ID（卡其）`）；顺序对不齐时不显示颜色，避免认错。
