...
下单余额低于 400 元时，仅在软件顶部显示红字提醒，不弹窗、不打断操作；刷新后余额达到 400 元会自动隐藏提醒。Windows 和 macOS 均支持。
...
待编辑和待付款页面中的 VMI 订单卡片提供“修改件数”按钮。点击订单后，弹窗会列出该订单的全部 SKU；直接在每行数量框中修改，点击“确认”后统一提交本单的变更。提交只替换备货数量，商品、颜色和尺码沿用订单原值；失败时弹窗和运行日志会列出订单号、SKU 及接口返回原因。

VMI 订单卡片下方直接列出每个 SKU 的颜色、尺码和下单数量，数量加粗显示，无需打开弹窗查看。Windows 和 macOS 的待编辑、待付款页面均支持；修改件数并刷新订单后，明细会同步更新。接口没有返回数量时显示“未返回”。
